...
const extractZipTo = async (source, dirname, filename) => {
  try {
    ...
    fs.readdir(pathDirectory, (err, files) => {
      ...
      let haveDirectories = files
        .map((item) => (fs.lstatSync(path.join(dirname, '/tmp/', filename, '/', item)).isDirectory() ? undefined : item))
        .includes(undefined);

      if (haveDirectories) {
        // el zip tiene directorios . FORMAT ERROR
        throw 'ZIP FORMAT ERROR.';
      }
      // comprobar la extension de todos los ficheros
      let haveIncorrectExtensions = files
        .map((item) => path.extname(item) != FILE_EXTENSIONS[0] && FILE_EXTENSIONS.includes(path.extname(item)))
        .includes(false);
